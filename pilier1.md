...
sequenceDiagram
    autonumber
    participant Infant as Infant (Proto-Soi)
    participant Tuteur as Tuteur (Extension Exécutive)
    Note over Infant: Niveau I: Élan somatique (Action X)
    Infant->>Tuteur: Déploiement de l'Action X (Modèle prédictif)
    Note over Tuteur: Niveau II: Posture Normative (Scorekeeping)
    Tuteur-->>Infant: Inhibition Attentionnelle Sociale (Veto Dynamique)
    Note over Infant: OP_DOUBLE_ERR / DOUBLE-ERREUR
    Note over Infant: Effondrement du modèle d'attente intersubjectif
    Note over Infant: Ré-indexation de l'erreur sur l'élan propre
    Infant->>Infant: Auto-Inhibition (Émergence du "Devoir-ne-pas-faire")
