# Opérateurs Logico-Formels   - Les fondations logico-mathématiques

Ce document pose les bases issues de la philosophie analytique et de la logique formelle classique (opérateurs propositionnels, modaux, dynamiques, etc.). Il sert de pont pour amener l'hypothèse centrale du système : l'opérateur n'est pas qu'un outil de calcul, il est "constitutif de l’être même du système".
## Approche Analytique
...
### 5.4 L'Opérateur de Stabilisation (\circlearrowright)
Introduire un opérateur spécifique pour le Protokin :
\circlearrowright x : "x s'auto-entretient par son opération".
Cet opérateur est le moteur de la Transduction (D3) et de l'Invariance (D1).
