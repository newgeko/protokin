...
- Point d'origine des tensions T2 et T4 face à l'émergence des normes et des raisons (R11).
...
- Réduction abusive (T1) : affirmer que l'intentionnalité collective (R8) ou le langage (R13) ne sont que de simples prolongements de la motricité spatiale.
...
- R3 — Ajustement allostatique : Fournit le support biologique continu et l'anticipation viscérale nécessaires aux capacités d'action.
- R5 — Minimisation de la surprise : Offre une formalisation probabiliste (FEP) des invariants stabilisés par la boucle sensorimotrice.

Traductions stables

- R4 ↔ R7 : Le couplage structurel (Varela) opère au niveau biologique global, R4 spécifie l'émergence de la cartographie environnementale cognitive.

Frictions cartographiées

- R6 — Récursion prospective : Tension issue du déplacement d'une régulation située vers une simulation abstraite (indépendante de l'action immédiate).
- R8 — Intentionnalité partagée : Tension de traduction (T2) et d'échelle face à la nécessité de décentrer l'invariant du sujet pour le placer dans l'intersubjectivité.

Incompatibilités structurelles

- R11 — Rupture épistémologique : Incompatibilité radicale. L'entrée dans l'espace des raisons (justifications normatives) exige de s'arracher à l'immanence de l'action située, ce qui induit une tension normative (T4) majeure.
...
La tension normative (T4) entre R4 et R11 rend saillante la différence fondamentale entre stabiliser une perception par le filtrage réussi de l'environnement (R4) et stabiliser une proposition par l'engagement dans le jeu des raisons (R11). Elle empêche l'illusion d'une continuité lisse entre la réaction sensorimotrice et la pensée abstraite justificative.
...
- Tension secondaire : T2 (Tension de traduction face à la coopération R8)

Transitions fréquemment observées

- R4 → R5 par réinterprétation (formalisation statistique bayésienne).
- R4 → R8 par émergence (ouverture au référencement social et à l'attention conjointe).
...
- Niveau 2 : Émergence
- Niveau 3 : Rupture
- Niveau 4 : Blocage OVM (Le passage R4 → R13 direct est formellement bloqué par la tension T5, exigeant l'étape socio-développementale R8/R9).

Rôle de l’OVM

L’OVM ne crée pas les limites du régime.

Il détecte les violations de frontières descriptives. L'OVM bloque fermement la dérive consistant à traiter le "démon de von Foerster" comme la genèse causale directe de la normativité, forçant l'observateur à assumer la reconfiguration de cadre qu'exige la rupture sellarsienne (R11).
...
Fichier : R4_competence_topographique_von_foerster.md

Connexions principales : R3, R5, R6, R8, R11
